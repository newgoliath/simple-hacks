...
# create intermediary key
echo --- creating the INTERMEDIARY KEY
#openssl genrsa \#h-aes256 \
openssl genrsa -out ${INTER_DIR}/private/intermediary.key.pem 4096
#      -passout pass:intermed_key \

# create intermediary cert

# create cert signing request
echo --- creating the INTERMEDIARY CSR
openssl req -config ${DIR}/intermediary_ca_openssl.cnf -new -sha256 \
      -key ${INTER_DIR}/private/intermediary.key.pem \
      -out ${INTER_DIR}/csr/intermediary.csr.pem
...
openssl ca -config ${DIR}/root_ca_openssl.cnf -extensions v3_intermediary_ca \
      ...
      -in ${INTER_DIR}/csr/intermediary.csr.pem \
      -out ${INTER_DIR}/certs/intermediary.cert.pem
...
openssl verify -CAfile ${CA_DIR}/certs/ca.cert.pem \
      ${INTER_DIR}/certs/intermediary.cert.pem
